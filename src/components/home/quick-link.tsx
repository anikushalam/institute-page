...
const QuickLink = ({ quickLinks }: { quickLinks: any }) => {
  return (
    <div className="flex flex-col w-[25%] h-[22]">
      <SubHeading className="font-extrabold text-[1.2rem] -mt-0 mb-2">
        Quick Links
      </SubHeading>
      ...
      <div className="flex flex-col w-full">
        {quickLinks?.link_1?.name && (
          <Link
            ...
            target="_blank"
            className=" border-l-4 border-prime px-3 py-2 my-1 bg-gray-200 hover:text-prime w-full md:w-11/12 block"
          >
            {quickLinks?.link_1?.name}
          </Link>
        )}
